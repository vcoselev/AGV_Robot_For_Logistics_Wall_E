...
const uint16_t QTRDefaultTimeout = 2500;
...
class QTRSensors
{
  public:
    ...
    void setMultPins(uint8_t sel_0, uint8_t sel_1);
  ...
  private:
    ...
    uint16_t _timeout = QTRDefaultTimeout; // only used for RC sensors
    uint16_t _maxValue = QTRDefaultTimeout; // the maximum value returned by readPrivate()
    uint8_t _samplesPerSensor = 6; // only used for analog sensors
